...
import '../domain/property_file_document_reference.dart';
import '../domain/property_file_reference.dart';
import 'link_document_to_property_file_dialog.dart';

/// Shows a property file with the documents linked to it.
class PropertyFileDetailsDialog extends StatefulWidget {
  const PropertyFileDetailsDialog({
  ...
  final PropertyFileReference propertyFile;

  @override
  State<PropertyFileDetailsDialog> createState() =>
      _PropertyFileDetailsDialogState();
}

class _PropertyFileDetailsDialogState extends State<PropertyFileDetailsDialog> {
  late final ApiRealEstateDashboardRepository _repository;
  List<PropertyFileDocumentReference> _documents = const [];
  bool _isBusy = false;
  String? _message;
  ...
  void initState() {
    super.initState();
    _repository = ApiRealEstateDashboardRepository(
      ...
      child: ConstrainedBox(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                widget.propertyFile.title,
                style: Theme.of(context).textTheme.headlineSmall,
              ),
              const SizedBox(height: 8),
              Text(
                '${widget.propertyFile.code} · ${widget.propertyFile.operationType} · ${widget.propertyFile.address}',
              ),
  ...
  Future<void> _loadDocuments() async {
    ...
    try {
      final documents = await _repository.loadPropertyFileDocuments(
        widget.propertyFile.id,
      );
      if (!mounted) {
        return;
      }
  ...
  Future<void> _openLinkDialog() async {
    await showDialog<void>(
      context: context,
      builder: (_) => LinkDocumentToPropertyFileDialog(
        sessionViewModel: widget.sessionViewModel,
        propertyFileId: widget.propertyFile.id,
  ...
  Future<void> _openDocument(PropertyFileDocumentReference item) {
