...
void main() {
  ...
  test('loadDocumentTypes carga solo tipos activos y deja mensaje', () async {
    ...
    await viewModel.loadDocumentTypes();

    expect(viewModel.documentTypes, hasLength(1));
    expect(viewModel.documentTypes.single.code, 'LEASE');
    expect(
      viewModel.message,
  ...
  test('upload informa cuando no hay sesion autenticada', () async {
    final client = MockClient((request) async {
      return http.Response('not-found', 404);
    });
    final sessionViewModel = AppSessionViewModel(httpClient: client);
  ...
  test('upload valida archivo vacio antes de intentar la subida', () async {
    final client = MockClient((request) async {
      if (request.url.path == '/api/auth/token') {
        return http.Response(
          jsonEncode({
